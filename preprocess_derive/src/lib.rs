...
fn parse(input: TokenStream) -> TokenStream {
	...
	// let _global_args = match parse_attributes(attrs, ident.to_string(),
	// false) { 	Ok(attrs) => attrs,
	// 	Err(err) => {
	...
	// 				if let Some(ident) = ident {
	// 					quote! {
	// 						#vis #ident: <#last_preprocessor_name as
	// preprocess::PreProcessor>::Processed, 					}
	// 				} else {
	// 					quote! {
	...
	// quote! {
	// 	impl #impl_generics preprocess::PreProcessor for #ident #ty_generics
	// #where_clause { 		type Args = ();
	// 		type Processed = #processed_type_name;

	// 		fn preprocess(self) -> Result<Self::Processed,
	// preprocess::PreProcessError> { 			#processor
	// 		}
	// 	}
